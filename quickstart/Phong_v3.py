    ...
    def run(self, delay, rebuf, video_size, end_of_video, play_video_id, Players, first_step=False):
        ...
        if play_video_id < 6:
            ...
            for seq in range(1,min(len(Players), PLAYER_NUM)):
                ...
                b_max_next=max(cond_p*((max(future_chunks_highest_size[seq])/1000000)/self.future_bandwidth),3.5*math.e**(-0.3*self.future_bandwidth-0.15*seq))
        ...
        # for seq in range(min(len(Players), PLAYER_NUM)):
            ...
            cond_p= float(user_retent_rate[Players[next_id].get_chunk_counter()]) / float(user_retent_rate[start_chunk])
            b_max_next=max(cond_p*((max(future_chunks_highest_size[next_id])/1000000)/self.future_bandwidth),1+TAU/1000)


        start_chunk = int(Players[0].get_play_chunk())
        _, user_retent_rate = Players[0].get_user_model()
        cond_p= float(user_retent_rate[Players[0].get_chunk_counter()]) / float(user_retent_rate[start_chunk])
        ...
        b_max=max(cond_p*((max(future_chunks_highest_size[0])/1000000)/self.future_bandwidth),3.5*math.e**(-0.3*self.future_bandwidth-0.15*0))
            # print("threshold2: ")
        ...
        if download_video_id == -1:  # no need to download
            ...
            #     bit_rate = 2
            # else:
            bit_rate = mpc_module.mpc(self.past_bandwidth, self.past_bandwidth_ests, self.past_errors, all_future_chunks_size[download_video_seq], P[download_video_seq], buffer_size, chunk_sum, video_chunk_remain, last_quality, Players, download_video_id, play_video_id,self.future_bandwidth)
                # bit_rate = 0
